...
californiaDEM_df <- as.data.frame(californiaDEM, xy = TRUE) %>%
  rename("elevation" = "file4f8c30565435",
         "lat" = "y",
         "long" = "x") %>%
  na.omit(elevation)
...
mapRaster <- californiaDEM_df %>%
  ...
  annotate("text", x = -119.3, y = 41.409, label = "Mount Shasta\n4,317 meters\nand\nShastina\n3,758 meters",
           hjust = "center", family = "Smooch Sans", size = 3, color = "navyblue",
           lineheight = 0.2) +
  # Lassen
  annotate("segment", x = -121.508, y = 40.492, xend = -119.5, yend = 40.492, linewidth = 0.3, color = "#FFFFFF") +
  annotate("text", x = -118.9, y = 40.492, label = "Mount Lassen\n3,189 meters",
           hjust = "center", family = "Smooch Sans", size = 3, color = "navyblue",
           lineheight = 0.2) +
    scale_fill_viridis(option = "G") +
  labs(title = "California Volcanoes",
       subtitle = "3 volcanoes in California are greater than 3000 meters in elevation") +
  coord_sf() +
  theme_void() +
